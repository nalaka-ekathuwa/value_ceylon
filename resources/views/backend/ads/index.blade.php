    ...
    <div class="card">
        <div class="card-header">
            <form method="GET" class="d-flex">
                <select name="status" class="form-control form-control-sm mr-2" style="width:200px;"
                    ...
                    <tbody>
                        @foreach($ads as $key => $ad)
                            <tr>
                                ...
                                <td>
                                    ...
                                    @if($ad->status === 'active')
                                        ...
                                        @if($ad->days_remaining > 0)
                                            <span class="badge badge-inline badge-soft-success mt-1">{{ $ad->days_remaining }}
                                                {{ translate('days left') }}</span>
                                        @else
                                            <span class="badge badge-inline badge-soft-dark mt-1">{{ translate('Expired') }}</span>
                                        @endif
                                ...
                                <td class="text-right">
                                    <button type="button" class="btn btn-sm btn-soft-info btn-icon btn-circle mr-1"
                                        title="{{ translate('View Details') }}" onclick="showDetailsModal({{ $ad->id }})">
                                        <i class="las la-eye"></i>
                                    </button>
                                    @if($ad->status === 'pending_payment')
                                        <a href="{{ route('admin.seller_ads.approve', $ad->id) }}"
                                            class="btn btn-sm btn-soft-success btn-icon btn-circle" title="{{ translate('Approve') }}"
                                            onclick="return confirm('Approve this ad?')">
                                            <i class="las la-check"></i>
                                        </a>
                                        <button type="button" class="btn btn-sm btn-soft-danger btn-icon btn-circle"
                                            title="{{ translate('Reject') }}" onclick="showRejectModal({{ $ad->id }})">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <textarea name="reason" class="form-control" rows="3"
                                placeholder="{{ translate('Enter reason...') }}"></textarea>
                        </div>
                    </div>
...
@section('script')
    <script>
        ...
        function showDetailsModal(adId) {
            $('#details-modal-content').html('<div class="text-center py-5"><i class="las la-spinner la-spin" style="font-size: 3rem;"></i></div>');
            ...
            $.post('{{ route('admin.seller_ads.details') }}', {
                ...
                if ($('.demo-select2-custom').length > 0) {
                    $('.demo-select2-custom').select2({
                        minimumResultsForSearch: -1,
                        dropdownParent: $('#detailsModal')
                    });
                }
                if (typeof AIZ === 'object' && AIZ.uploader && typeof AIZ.uploader.previewGenerate === 'function') {
                    AIZ.uploader.previewGenerate();
                }
            }).fail(function () {
                $('#details-modal-content').html('<div class="alert alert-danger m-3">{{ translate("Failed to load details. Please try again.") }}</div>');
            });
        }
    </script>
@endsection