...
class PayhereController extends Controller
{
    ...
    */
    ...
    public static function seller_package_notify()
    {
        ...
        if (($local_md5sig === $md5sig) and ($status_code == 2)) {
            $user = User::where('id', $_POST['custom_1'])->with('shop')->first();

            $seller = $user->shop;
            $seller->seller_package_id = $_POST['custom_2'];
            $seller_package = SellerPackage::findOrFail($_POST['custom_2']);
            ...
            $seller_package->seller_package_id = $_POST['custom_2'];
            $seller_package->payment_method = $_POST['method'];
            $seller_package->payment_details = json_encode($_POST);
        ...
        return PayhereController::seller_package_incomplete();
    }

    public static function seller_package_cancel()
    {
        return PayhereController::seller_package_incomplete();
    }
    ...
    public static function seller_package_payment_return(){
        Session::forget('payment_data');
        flash(translate("Payment Complete"))->message();
        return redirect()->route('home')->send();
    }

    public static function seller_package_success($id, $customer_package_id, $payment)
    {
